...
public class TestJdbcDemo {
	...
	public static void main(String[] args) {
		...
		case 2->{
			List<MyProduct> plist=pservice.getAllProduct();
		...
		case 3->{
			...
			int cid=sc.nextInt();
			List<MyProduct> plist=pservice.getByCatId(cid);
			plist.forEach(System.out::println);
		}
		case 4->{
			System.out.println("enter product id");
			int pid=sc.nextInt();
			MyProduct p=pservice.getByPid(pid);
			if(p!=null) {
		...
		case 7->{
			List<MyProduct> plist=pservice.sorteByPrice();
			plist.forEach(System.out::println);
		}
		case 8->{
			sc.close();
