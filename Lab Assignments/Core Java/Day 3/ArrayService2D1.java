...
public class ArrayService2D1 {
	...
	public static int[] convertTo1DArray(int[][] arr) {
	    ...
	    public static int findMaxAllNumbers(int[][] arr) {
	        int max = arr[0][0];
	    ...
	    public static int[] findRowwiseMax(int[][] arr) {
	        ...
	        for (int i = 0; i < numRows; i++) {
	            ...
	            int currentRowMax = arr[i][0];
	            
	            for (int j = 1; j < arr[i].length; j++) {
	                if (arr[i][j] > currentRowMax) {
	                    currentRowMax = arr[i][j];
	                }
	            }
	            rowMaxes[i] = currentRowMax;
	        }
	    ...
	    public static int[] findColumnwiseMax(int[][] arr) {
	        ...
	        for (int i = 1; i < numRows; i++) {
	            int currentRowLength = arr[i].length;
	            
	            for (int j = 0; j < numCols; j++) {
	                if (j < currentRowLength) { 
	                    if (arr[i][j] > colMaxes[j]) {
	                        colMaxes[j] = arr[i][j];
	                    }
	                }
	            }
